...
public class MemoryUsageGaugeSetTest {
    ...
    private static final MetricName NON_HEAP = MetricName.build("non_heap");
    private static final MetricName POOLS = MetricName.build("pools");
    private static final MetricName BIG_POOL = POOLS.resolve("big_pool");
    private static final MetricName WEIRD_POOL = POOLS.resolve("weird_pool");
    ...
    public void hasASetOfGauges() throws Exception {
        assertThat(gauges.getMetrics().keySet())
                .containsOnly(
                        ...
                        // skip in non-collected pools - "pools.Big-Pool.used_after_gc",
                        WEIRD_POOL.resolve("init"),
                        WEIRD_POOL.resolve("committed"),
                        WEIRD_POOL.resolve("used"),
                        WEIRD_POOL.resolve("used_after_gc"),
    ...
    public void hasAGaugeForWeirdCollectionPoolUsed() throws Exception {
        final Gauge gauge = (Gauge) gauges.getMetrics().get(WEIRD_POOL.resolve("used_after_gc"));
